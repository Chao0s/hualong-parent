...
dynamic_growth_status_count (动态成长状态数) = 0:2
runtime_clickable_node_count (运行时可点击节点数) = 10
growth_record_page_removed (成长档案页已删) = 2026-08-02 评审删除 growth-record.html 与本页「成长档案」入口（decision.md 第 11 条）：其内容为素材聚合镜像，与全部活动/在园时光/成长册重复，且全应用只有本页一个入口。db_growth_record 仍是 Teacher canonical 对象、home-spec.md 报告流仍读它，只是家长端不再有专门页面
entry_rename_note (入口改名说明) = 「评价/评估报告」改为「评估报告」（decision.md 第 11 条），node_key 不变
...
shared_objects (共享对象) = db_school, db_class, db_child, db_month_eval, db_growth_book, db_parent_evaluation, db_child_assessment, db_child_assessment_item, db_parent, db_parent_child
...
static_ui_content (保留的静态界面内容) = 页面标题、基础信息字段标签、成长栏目名、两个固定入口、编辑说明、性别固定选项、底部导航和空状态文案
business_seed (生产环境业务种子数据) = NONE
production_initial_growth_objects (成长业务表初始状态) = db_growth_book|db_month_eval|db_parent_evaluation EMPTY
...
| 2 | 评估报告 | Evaluation Reports | btn_parent_profile_evaluation_report | db_month_eval + db_parent_evaluation + db_child_assessment | child_id | child-profile.html > evaluation-report.html?from=profile |
| 3 | 成长册 | Growth Book | btn_parent_profile_growth_book | db_growth_book | child_id | child-profile.html > growth-book.html?from=profile |
| 4 | 编辑层背景关闭 | Close Editor by Backdrop | btn_parent_profile_edit_backdrop_close | db_parent_child_profile_home | NULL | close profile-edit sheet |
| 5 | 编辑层关闭 | Close Profile Editor | btn_parent_profile_edit_close | db_parent_child_profile_home | NULL | close profile-edit sheet |
| 6 | 保存修改 | Save Profile Changes | btn_parent_profile_save | db_child | child_id, child_name, birth_date, gender | submit then close profile-edit sheet |
| 7 | 首页 | Home | nav_parent_home | nav_parent_home | NULL | home.html |
| 8 | 家园共育 | Home School | nav_parent_tasks | nav_parent_tasks | NULL | home-school.html |
| 9 | 在园时光 | Kindergarten Moments | nav_parent_moments | nav_parent_moments | NULL | kindergarten-moments.html |
| 10 | 儿童档案 | Child Profile | nav_parent_child_profile | nav_parent_child_profile | NULL | child-profile.html |
...
applies_to (适用范围) = 本页「评估报告」入口跳转的 evaluation-report.html，以及成长册的 comp 综合评估栏目
...
db_growth_book|db_month_eval = REUSE Teacher/05 home-school-spec.md and Teacher/01 home-spec.md（db_growth_record 已随成长档案页删除而移出本页范围，对象本身仍为 Teacher canonical）
...
IF evaluation report|growth book object_id NOT_FOUND, return 404
